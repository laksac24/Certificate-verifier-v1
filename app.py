...
# def process_human_clicked_images():
#     """
#     Process human-clicked images using object detection to extract certificates
#     """
...
#     # Get all image files from certificates folder
#     image_extensions = ['.jpg', '.jpeg', '.png', '.bmp', '.tiff']
#     image_files = []
    
#     for file in os.listdir(certificates_folder):
...
#         image_path = os.path.join(certificates_folder, image_file)
#         print(f"Processing {image_file} for certificate detection...")
...
#         detected_certificates.extend(cropped_paths)
    
#     return detected_certificates
...
#         error_msg = "No image available for processing. Please add a valid image or PDF to the certificates folder."
#         state["messages"].append({"role": "assistant", "content": error_msg})
#         return state
...
#             if("ecertificate" in classification.lower()):
#                 state["ecerti"].append(png_file)
#             else:
#                 state["human"].append(png_file)
                
#             print(f"Model response for {png_file}: {classification}")
...
# def certificate_type_llm(state: State):
    ...
    for png_file in png_files:
        ...
        prompt = [
            {"role": "system", "content": "You are an assistant that classifies certificate type. Dont give me any extra information, just tell me whether the certificate is ecertificate or normal human clicked imageof the certificate"},
            {"role": "user", "content": [
                {"type": "text", "text": "Classify this certificate:"},
        ...
        response = image_llm.invoke(prompt)
        classification = response.content
        if("ecertificate" in classification):
            state["ecerti"].append(png_file)
        else :
            state["human"].append(png_file)
        print(f"Model response for {png_file}: {classification}")
        
        # Add response for each certificate
        state["messages"].append({
            "role": "assistant", 
            "content": f"Certificate: {png_file} | Classification: {classification}"
        })
